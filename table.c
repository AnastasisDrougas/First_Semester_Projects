...
int answer_checkF4_a_n(int x1, int x2);//Answer check for F4 function(for atomic numbers).
float answer_checkF4_a_mass(float x1, float x2);//Answer check for F4 function(for atomic mass).
...
void F4(int answerF4,char chemical_group[118][50],char table[118][10],char name[118][50],char symbol[118][50],int atomic_number[118],double atomic_mass[118])//Fourth monitor.
{
    ...
    do
    {
       //Making the monitor.
        ...
        clearmonitor();
        con_rest=F5(min_a_n,max_a_n,cg,min_a_mass,max_a_mass,table,atomic_number,chemical_group,atomic_mass);
    }while(con_rest==1);
}
